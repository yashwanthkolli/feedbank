...
const TeacherNotificationPage = () => {
    ...
    useEffect(() => {
        if(params.id) {
            axios.get(`http://localhost:5000/notification/get/${params.id}`, { headers: { Authorization: `Bearer ${sessionStorage.getItem('token')}` } })
    ...
    useEffect(() => {
        axios.post(
            `http://localhost:5000/notification/get/all/${decodeSessionStorage().school_id}`,
            {
                offset: 7*(page-1),
                limit: 7
            },
